...
if admin.blank?
	User.create! :first_name => 'Super', :last_name => 'Admin', :email=>'msmukesh4+super_admin@gmail.com', :password => 'Instabookr@admin123', :password_confirmation => 'Instabookr@admin123', :confirmed_at => Time.now, :contact_number1 => '9040357009', :status => 'Active', is_super_user: true
end
...
if shop.blank?
	Shop.create! :name => "Bajrangi Gym", :user_id => User.find_by_email("msmukesh4+owner@gmail.com").id, :service_type_id => ServiceType.find_by_name("Spa").id, :uuid => "12345678900987654321", :time_slot_ids => "[8,9,10,11,12]", :day_slot_ids => "[1,2,3,4,5,6]", :status => "Active", :contact_number1 => '9876543210', :location_id => Location.find_by_location('New Town').id, :caption => "demo caption", :details => "demo details", :address => "demo address"
end
